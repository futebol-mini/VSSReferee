...
GLText::GLText(QFont _font) : font(_font) {
    glyphs.clear();
...
QVector2D GLText::getSize(char ch) {
    return {static_cast<float>(getWidth(ch)), static_cast<float>(getHeight(ch))};
}
...
void GLText::drawString(QVector2D loc, double angle, double size, const char *str,
...
void GLText::initializeGlyph(char ch) {
    ...
    for (int i = 0; i < polygons.size(); i++) {
        if (debugTesselation)
            printf("Sub-Polygon %d:\n", i);
        numVertices += polygons[i].size();

        for (auto k : polygons[i]) {
            if (debugTesselation)
                printf("%8.3f,%8.3f\n", k.x(), k.y());
    ...
    for (auto &polygon : polygons) {
        for (auto k : polygon) {
            vertices[j][0] = k.x() / FontRenderSize;
            vertices[j][1] = -k.y() / FontRenderSize;
            vertices[j][2] = 9;
            j++;
        }
    }
...
void GLText::tessErrorCB(GLenum errorCode) {
    const GLubyte *errorStr = nullptr;
