...
void debugReplacePosition(double x, double y, int num = 0, std::string side = "") {
    std::cout << Text::blue("[REPLACER] ", true) +
                     Text::bold("Moving player " + std::to_string(num) + " from team \"" + side +
                                "\" " + " to position (" + std::to_string(x) + ", " +
                                std::to_string(y) + ")") +
                     '\n';
}
...
Replacer::Replacer(QString replaceFileName, Vision *vision, Field *field, Constants *constants)
    : Entity(ENT_REPLACER), _constants(constants), _field(field),
      _firaPort(getConstants()->firaPort()), _replaceFileName(std::move(replaceFileName)),
      _replacerPort(getConstants()->replacerPort()), _vision(vision) {
    // Take pointers
...
void Replacer::bindAndConnect() {
    ...
    _firaClient->connectToHost(_firaAddress, _firaPort, QIODevice::WriteOnly,
                               QAbstractSocket::IPv4Protocol);

    // Binding replacer in defined network data
    if (_replacerClient->bind(QHostAddress(_replacerAddress), _replacerPort,
    ...
    if (_replacerClient->joinMulticastGroup(
            QHostAddress(_replacerAddress),
            QNetworkInterface::interfaceFromName(getConstants()->networkInterface())) == false) {
        std::cout << Text::blue("[REPLACER] ", true)
                  << Text::red("Error while joining multicast.", true) + '\n';
        return;
    }
}
...
void Replacer::initialization() {
    ...
    // Debug network info
    std::cout << Text::blue("[REPLACER] ", true) +
                     Text::bold("Module started at address '" + _replacerAddress.toStdString() +
                                "' and port '" + std::to_string(_replacerPort) + "'.") +
                     '\n';
}
...
void Replacer::loop() {
    while (_replacerClient->hasPendingDatagrams()) {
        ...
        if (frame.ParseFromArray(datagram.data().data(), datagram.data().size()) == false) {
            std::cout << Text::blue("[REPLACER] ", true)
                      << Text::red("Frame packet parsing error.", true) + '\n';
            continue;
        }
        ...
        if (_placementStatus.value(VSSRef::Color::BLUE) == true &&
            _placementStatus.value(VSSRef::Color::YELLOW) == true) {
...
void Replacer::parsePlacements() {
    ...
    for (const auto &foul : fouls) {
        ...
        QMap<QString, QVariant> foulsMap = documentMap()[foul].toMap();
        QList<QString> categories = foulsMap.keys();

        // Register foul at placement map
        if (!placements.contains(foul)) {
            placements.insert(foul, new QMap<QString, QMap<QString, QVector<PlaceData>> *>());
        }
        QMap<QString, QMap<QString, QVector<PlaceData>> *> *foulPlacementMap =
            placements.value(foul);

        // For each category
        for (const auto &categorie : categories) {
            // Take roles
            QMap<QString, QVariant> categoriesMap = foulsMap[categorie].toMap();
            QList<QString> roles = categoriesMap.keys();

            // Register category at placement map
            if (!foulPlacementMap->contains(categorie)) {
                foulPlacementMap->insert(categorie, new QMap<QString, QVector<PlaceData>>());
            }
            QMap<QString, QVector<PlaceData>> *categoriesPlacementMap =
                foulPlacementMap->value(categorie);

            // For each role
            for (const auto &role : roles) {
                // Take placement types data
                QMap<QString, QVariant> rolesMap = categoriesMap[role].toMap();
                QList<QString> placementTypes = rolesMap.keys();
                QVector<PlaceData> placementsData;

                // For each placement type
                for (const auto &placementType : placementTypes) {
                    // Take placement data
                    QMap<QString, QVariant> placementInfo = rolesMap[placementType].toMap();

                    // Get PlaceData
                    PlaceData placeData(
                        Position(true, placementInfo["x"].toFloat(), placementInfo["y"].toFloat()),
                        Angle(true, placementInfo["ori"].toFloat()));
                ...
                categoriesPlacementMap->insert(role, placementsData);
            }
        }
    }
}
...
QMap<QString, QVector<PlaceData>> Replacer::getPlacementsByFoul(QString foul) {
    ...
    if (!placements.contains(foul)) {
        std::cout << Text::red("[ERROR] ", true) +
                         Text::bold("The foul '" + foul.toStdString() +
                                    "' cannot be found in placement list. Please check the JSON "
    ...
    QMap<QString, QMap<QString, QVector<PlaceData>> *> *placementsByCategory =
        placements.value(foul);
    ...
    if (!placementsByCategory->contains(category)) {
        std::cout << Text::red("[ERROR] ", true) +
                         Text::bold("The category '" + category.toStdString() +
                                    "' cannot be found in foul '" + foul.toStdString() +
                                    "'placement categories list. Please check the JSON file to "
                                    "avoid mistakes.") +
                         '\n';
        return {};
    }
...
VSSRef::Frame Replacer::getPlacementFrameByFoul(QString foul, VSSRef::Quadrant foulQuadrant,
    ...
    bool teamIsAtLeft = (teamColor == VSSRef::Color::BLUE && getConstants()->blueIsLeftSide()) ||
                        (teamColor == VSSRef::Color::YELLOW && !getConstants()->blueIsLeftSide());

    // Check if team will make the kick
    bool teamWillMakeTheKick = (teamColor == getFoulColor()) ||
                               ((teamIsAtLeft && (foulQuadrant == VSSRef::Quadrant::QUADRANT_1 ||
                                                  foulQuadrant == VSSRef::Quadrant::QUADRANT_4)) ||
                                (!teamIsAtLeft && (foulQuadrant == VSSRef::Quadrant::QUADRANT_2 ||
                                                   foulQuadrant == VSSRef::Quadrant::QUADRANT_3)));
    ...
    if (foul == "GOAL_KICK" && getFoulColor() == teamColor) {
        ...
        gkPosition = Position(true, gkPosition.x(),
                              (_isGoaliePlacedAtTop) ? gkPosition.y() : -gkPosition.y());

        // Future ball position
        Position futureBall =
            getBallPlaceByFoul(VSSRef::Foul::GOAL_KICK, teamColor, VSSRef::Quadrant::NO_QUADRANT);

        // Move
        movePlayerToPosition(frame.add_robots(), goalieId, gkPosition,
                             Angle(true, Utils::getAngle(futureBall, gkPosition)));
    } else {
        movePlayerToPosition(frame.add_robots(), goalieId, goalKeeperPlaceData.getPosition(),
                             goalKeeperPlaceData.getOrientation());
    }
    ...
    if (foul == "FREE_BALL") {
        ...
        Position fbPosition =
            getBallPlaceByFoul(VSSRef::Foul::FREE_BALL, VSSRef::Color::NONE, getFoulQuadrant());

        // Get x-axis diff
        float xAxisDiff = (getConstants()->is5v5()) ? 0.25 : 0.2;

        // Place attacker in correct position
        movePlayerToPosition(frame.add_robots(), attackerId,
                             Position(true,
                                      fbPosition.x() + (teamIsAtLeft ? -xAxisDiff : xAxisDiff),
                                      fbPosition.y()),
                             Angle(true, 0.0f));
    }
...
bool Replacer::checkIfCollides(VSSRef::Frame blueFrame, VSSRef::Frame yellowFrame) {
    ...
    for (int i = 0; i < blueFrame.robots_size(); i++) {
        for (int j = 0; j < yellowFrame.robots_size(); j++) {
            Position blueRobotPosition =
                Position(true, blueFrame.robots(i).x(), blueFrame.robots(i).y());
            Position yellowRobotPosition =
                Position(true, yellowFrame.robots(j).x(), yellowFrame.robots(j).y());

            if (Utils::distance(blueRobotPosition, yellowRobotPosition) <=
                1.1 * getConstants()->robotLength()) {
                std::cout << Text::yellow("[REPLACER] ", true) +
                                 Text::bold("Detected collision with blue and yellow frames:") +
                                 '\n';
                std::cout << "Collision with blue robot " +
                                 std::to_string(blueFrame.robots(i).robot_id()) +
                                 " at position x: " + std::to_string(blueRobotPosition.x()) +
                                 " y: " + std::to_string(blueRobotPosition.y()) + '\n';
                std::cout << "Collision with yellow robot " +
                                 std::to_string(yellowFrame.robots(j).robot_id()) +
                                 " at position x: " + std::to_string(yellowRobotPosition.x()) +
                                 " y: " + std::to_string(yellowRobotPosition.y()) + '\n';
    ...
    for (int i = 0; i < yellowFrame.robots_size(); i++) {
        for (int j = 0; j < yellowFrame.robots_size(); j++) {
            ...
            Position yellowRobotPosition =
                Position(true, yellowFrame.robots(i).x(), yellowFrame.robots(i).y());
            Position yellowRobotPosition2 =
                Position(true, yellowFrame.robots(j).x(), yellowFrame.robots(j).y());

            if (Utils::distance(yellowRobotPosition, yellowRobotPosition2) <=
                1.1 * getConstants()->robotLength()) {
                std::cout << Text::yellow("[REPLACER] ", true) +
                                 Text::bold("Detected collision at yellow frame:") + '\n';
                std::cout << "Collision with yellow robot " +
                                 std::to_string(yellowFrame.robots(i).robot_id()) +
                                 " at position x: " + std::to_string(yellowRobotPosition.x()) +
                                 " y: " + std::to_string(yellowRobotPosition.y()) + '\n';
                std::cout << "Collision with yellow robot " +
                                 std::to_string(yellowFrame.robots(j).robot_id()) +
                                 " at position x: " + std::to_string(yellowRobotPosition2.x()) +
                                 " y: " + std::to_string(yellowRobotPosition2.y()) + '\n';
    ...
    for (int i = 0; i < blueFrame.robots_size(); i++) {
        for (int j = 0; j < blueFrame.robots_size(); j++) {
            ...
            Position blueRobotPosition =
                Position(true, blueFrame.robots(i).x(), blueFrame.robots(i).y());
            Position blueRobotPosition2 =
                Position(true, blueFrame.robots(j).x(), blueFrame.robots(j).y());

            if (Utils::distance(blueRobotPosition, blueRobotPosition2) <=
                1.1 * getConstants()->robotLength()) {
                std::cout << Text::yellow("[REPLACER] ", true) +
                                 Text::bold("Detected collision at blue frame:") + '\n';
                std::cout << "Collision with blue robot " +
                                 std::to_string(blueFrame.robots(i).robot_id()) +
                                 " at position x: " + std::to_string(blueRobotPosition.x()) +
                                 " y: " + std::to_string(blueRobotPosition.y()) + '\n';
                std::cout << "Collision with blue robot " +
                                 std::to_string(blueFrame.robots(j).robot_id()) +
                                 " at position x: " + std::to_string(blueRobotPosition2.x()) +
                                 " y: " + std::to_string(blueRobotPosition2.y()) + '\n';
...
void Replacer::takeFoul(VSSRef::Foul foul, VSSRef::Color foulColor, VSSRef::Quadrant foulQuadrant) {
    if (foul == VSSRef::Foul::GAME_ON || foul == VSSRef::Foul::STOP)
        return;
...
Position Replacer::getBallPlaceByFoul(VSSRef::Foul foul, VSSRef::Color color,
    ...
    case VSSRef::Foul::GOAL_KICK: {
        ...
        if (color == VSSRef::Color::BLUE) {
            if (getConstants()->blueIsLeftSide())
                return {true, -goalKickX,
                        ((_isGoaliePlacedAtTop) ? (stretch - getConstants()->ballRadius())
                                                : (-stretch + getConstants()->ballRadius()))};
            else
                return {true, goalKickX,
                        ((_isGoaliePlacedAtTop) ? (stretch - getConstants()->ballRadius())
                                                : (-stretch + getConstants()->ballRadius()))};
    ...
    case VSSRef::Foul::PENALTY_KICK: {
        float penaltyMarkX =
            getConstants()->is5v5() ? (getField()->fieldLength() / 2000.0 - 0.375) : markX;
        if (color == VSSRef::Color::BLUE) {
            if (getConstants()->blueIsLeftSide())
                return {true, penaltyMarkX, 0};
            else
                return {true, -markX, 0.0};
        } else if (color == VSSRef::Color::YELLOW) {
            if (getConstants()->blueIsLeftSide())
                return {true, -penaltyMarkX, 0};
            else
...
VSSRef::Frame Replacer::getOutsideFieldPlacement(VSSRef::Color teamColor) {
    ...
    // Get field data
    float sideLength = (getField()->fieldLength() / 1000.0) / 2.0 -
                       (players.size() * 2 * getConstants()->robotLength());
    float fieldWidth = (getField()->fieldWidth() / 1000.0) / 2.0;

    // Place each robot outside field
    for (int i = 0; i < players.size(); i++) {
        PlaceData toPlace(Position(true,
                                   ((i + 1) * 2 * getConstants()->robotLength()) +
                                       (i * sideLength / players.size()),
                                   1.1 * fieldWidth),
                          Angle(true, Angle::pi / 2.0));
        if (teamIsAtLeft)
            toPlace.reflect();
        movePlayerToPosition(frame.add_robots(), players.at(i), toPlace.getPosition(),
                             toPlace.getOrientation());
    }
...
VSSRef::Frame Replacer::getPenaltyShootoutPlacement(VSSRef::Color teamColor, bool placeAttacker) {
    ...
    if (placeAttacker) {
        ...
        // Take ball position
        Position ballPosition = getBallPlaceByFoul(VSSRef::Foul::PENALTY_KICK, teamColor,
                                                   VSSRef::Quadrant::NO_QUADRANT);

        // Take team placement frame (getPenaltyShootoutPlacement will only be call after
        // placeTeams())
    ...
    for (int i = 0; i < frameOutside.robots_size(); i++) {
        ...
        if (robotAt.robot_id() != chosenId) {
            movePlayerToPosition(frame.add_robots(), robotAt.robot_id(),
                                 Position(true, robotAt.x(), robotAt.y()),
                                 Angle(true, Angle::toRadians(robotAt.orientation())));
        } else {
            movePlayerToPosition(frame.add_robots(), chosenRobot.robot_id(),
                                 Position(true, chosenRobot.x(), chosenRobot.y()),
                                 Angle(true, Angle::toRadians(chosenRobot.orientation())));
        }
    }
...
void Replacer::placeFrame(VSSRef::Frame frame) {
    ...
    auto *command = new fira_message::sim_to_ref::Replacement();
    std::string msg;
    ...
    for (int i = 0; i < frame.robots_size(); i++) {
        ...
        auto *robotPosition = new fira_message::Robot();
        ...
        if (frame.teamcolor() == getFoulColor()) {
            if (Utils::isInsideGoalArea(frame.teamcolor(),
                                        Position(true, frameRobot.x(), frameRobot.y()))) {
                if (frameRobot.y() >= 0) {
    ...
    if (_firaClient->write(msg.c_str(), msg.length()) == -1) {
        std::cout << Text::blue("[REPLACER] ", true) +
                         Text::red("FiraClient failed to write to socket.", true) + '\n';
    }
}
...
void Replacer::placeBall(Position ballPos, Velocity ballVelocity) {
    ...
    auto *ballPlacement = new fira_message::sim_to_ref::BallReplacement();
...
void Replacer::placeTeams(bool forceDefault, bool isToPlaceOutside) {
    ...
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        ...
        // if team not placed, take default positions
            ...
            // Take default frame
            VSSRef::Frame defaultFrame = getPlacementFrameByFoul(
                VSSRef::Foul_Name(lastFoul).c_str(), getFoulQuadrant(), VSSRef::Color(i));
    ...
    if ((lastFoul == VSSRef::Foul::KICKOFF || lastFoul == VSSRef::Foul::PENALTY_KICK) &&
        isToPlaceOutside) {
        // Cast place outside
        VSSRef::Color oppositeColor =
            (lastFoulColor == VSSRef::BLUE) ? VSSRef::YELLOW : VSSRef::BLUE;
...
void Replacer::placeOutside(VSSRef::Foul foul, VSSRef::Color oppositeTeam) {
    if (foul == VSSRef::Foul::KICKOFF) {
        ...
        // Filling frames
        removedFrameKicker = getPenaltyShootoutPlacement(
            (oppositeTeam == VSSRef::Color::BLUE) ? VSSRef::Color::YELLOW : VSSRef::Color::BLUE,
            true);
        removedFrameGoalie = getPenaltyShootoutPlacement(oppositeTeam, false);

        // Set teams frames
        setTeamFrame((oppositeTeam == VSSRef::Color::BLUE) ? VSSRef::Color::YELLOW
                                                           : VSSRef::Color::BLUE,
                     removedFrameKicker);
    ...
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        if (_lastFrame.contains(VSSRef::Color(i))) {
            ...
            for (unsigned char key : keys) {
                // Take robot
                fira_message::Robot *robot = hash->value(key);
...
void Replacer::saveFrameAndBall() {
    ...
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        ...
        for (unsigned char avPlayer : avPlayers) {
            ...
            if (!hash->contains(avPlayer)) {
                hash->insert(avPlayer, new fira_message::Robot());
            }

            // Taking data from vision
            Position robotPosition = _vision->getPlayerPosition(VSSRef::Color(i), avPlayer);
            Angle robotOrientation = _vision->getPlayerOrientation(VSSRef::Color(i), avPlayer);
            Velocity robotVelocity = _vision->getPlayerVelocity(VSSRef::Color(i), avPlayer);

            // Filling robot data
            fira_message::Robot *robotData = hash->value(avPlayer);
            robotData->set_robot_id(avPlayer);
...
void Replacer::placeLastFrameAndBall() {
    ...
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        ...
        for (unsigned char avPlayer : avPlayers) {
            ...
            if (!lastTeamFrame->contains(avPlayer)) {
                continue;
            }
            ...
            robot->CopyFrom(*lastTeamFrame->value(avPlayer));
...
Field *Replacer::getField() {
    if (_field == nullptr) {
        std::cout << Text::red("[ERROR] ", true)
                  << Text::bold("Field with nullptr value at Replacer") + '\n';
    } else {
        return _field;
    }
...
Constants *Replacer::getConstants() {
    if (_constants == nullptr) {
        std::cout << Text::red("[ERROR] ", true)
                  << Text::bold("Constants with nullptr value at Replacer") + '\n';
    } else {
        return _constants;
    }
