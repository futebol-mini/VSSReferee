...
QString Checker_TwoDefenders::name() { return "Checker_TwoDefenders"; }
...
void Checker_TwoDefenders::run() {
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        ...
        if ((isTwoPlayersDefendingAtGoalArea(VSSRef::Color(i)) &&
             Utils::isInsideGoalArea(VSSRef::Color(i), ballPosition)) ||
            (isFivePlayersDefendingAtBigArea(VSSRef::Color(i)) &&
             Utils::isInsideBigArea(VSSRef::Color(i), ballPosition))) {
            _timers.value(VSSRef::Color(i))->stop();

            if (_timers.value(VSSRef::Color(i))->getSeconds() >=
                    getConstants()->ballInAreaMaxTime() &&
                !getConstants()->useRefereeSuggestions()) {
                setPenaltiesInfo(VSSRef::Foul::PENALTY_KICK, oppositeColor,
...
bool Checker_TwoDefenders::isTwoPlayersDefendingAtGoalArea(VSSRef::Color teamColor) {
    ...
    for (unsigned char avPlayer : avPlayers) {
        Position playerPosition = getVision()->getPlayerPosition(teamColor, avPlayer);
        if (Utils::isInsideGoalArea(teamColor, playerPosition) &&
            Utils::isInsideGoalArea(teamColor, ballPosition)) {
...
bool Checker_TwoDefenders::isFivePlayersDefendingAtBigArea(VSSRef::Color teamColor) {
    ...
    for (unsigned char avPlayer : avPlayers) {
        Position playerPosition = getVision()->getPlayerPosition(teamColor, avPlayer);
        if (Utils::isInsideBigArea(teamColor, playerPosition) &&
            Utils::isInsideBigArea(teamColor, ballPosition)) {
            countAtAllieGoal = countAtAllieGoal + 1;
        }
    }
