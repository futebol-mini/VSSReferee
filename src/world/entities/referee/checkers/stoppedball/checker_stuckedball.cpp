...
QString Checker_StuckedBall::name() { return "Checker_StuckedBall"; }
...
void Checker_StuckedBall::setIsPenaltyShootout(bool isPenaltyShootout,
...
void Checker_StuckedBall::run() {
    ...
    if (!getVision()->getBallPosition().isInvalid() &&
        getVision()->getBallVelocity().abs() <= getConstants()->ballMinSpeedForStuck()) {
        // Take ball position
        ...
        for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
            if (Utils::isInsideGoalArea(VSSRef::Color(i), ballPosition)) {
                ...
                if (_timer.getSeconds() >= getConstants()->stuckedBallTime() &&
                    ...
                    if (havePlayersNearlyBall(VSSRef::Color::BLUE) &&
                        havePlayersNearlyBall(VSSRef::Color::YELLOW)) {
                        setPenaltiesInfo(VSSRef::Foul::GOAL_KICK, VSSRef::Color(i),
                                         VSSRef::Quadrant::NO_QUADRANT);
                    } else {
                        setPenaltiesInfo(VSSRef::Foul::PENALTY_KICK,
                                         ((i == VSSRef::Color::BLUE) ? VSSRef::Color::YELLOW
                                                                     : VSSRef::Color::BLUE),
                                         VSSRef::Quadrant::NO_QUADRANT);
                    }
                    emit foulOccured();
        ...
        if (!isAtGoalAreas) {
            ...
            if (_timer.getSeconds() >= getConstants()->stuckedBallTime()) {
                if (_isPenaltyShootout) {
                    ...
                    // Set penalties and emit that an foul occured
                    setPenaltiesInfo(VSSRef::Foul::FREE_BALL, VSSRef::Color::NONE,
                                     Utils::getBallQuadrant(ballPosition));
                    emit foulOccured();
                }
