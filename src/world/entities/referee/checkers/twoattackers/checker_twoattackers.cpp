...
QString Checker_TwoAttackers::name() { return "Checker_TwoAttackers"; }
...
void Checker_TwoAttackers::run() {
    for (int i = VSSRef::Color::BLUE; i <= VSSRef::Color::YELLOW; i++) {
        ...
        if ((isTwoPlayersAttackingAtGoalArea(VSSRef::Color(i)) &&
             Utils::isInsideGoalArea(oppositeColor, ballPosition)) ||
            (isFourPlayersAttackingAtBigArea(VSSRef::Color(i)) &&
             Utils::isInsideBigArea(oppositeColor, ballPosition))) {
            ...
            if (_timers.value(VSSRef::Color(i))->getSeconds() >=
                ...
                setPenaltiesInfo(VSSRef::Foul::GOAL_KICK, oppositeColor,
...
bool Checker_TwoAttackers::isTwoPlayersAttackingAtGoalArea(VSSRef::Color teamColor) {
    ...
    VSSRef::Color oppositeColor =
        (teamColor == VSSRef::Color::BLUE) ? VSSRef::Color::YELLOW : VSSRef::Color::BLUE;

    // Count opposite players at goal
    int countAtOppositeGoal = 0;
    for (unsigned char avPlayer : avPlayers) {
        Position playerPosition =
            getVision()->getPlayerPosition(VSSRef::Color(teamColor), avPlayer);
...
bool Checker_TwoAttackers::isFourPlayersAttackingAtBigArea(VSSRef::Color teamColor) {
    ...
    for (unsigned char avPlayer : avPlayers) {
        Position playerPosition =
            getVision()->getPlayerPosition(VSSRef::Color(teamColor), avPlayer);
        if (Utils::isInsideBigArea(oppositeColor, playerPosition) &&
            Utils::isInsideBigArea(oppositeColor, ballPosition)) {
            countAtOppositeGoal = countAtOppositeGoal + 1;
        }
    }
