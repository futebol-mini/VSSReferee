...
Matrix::Matrix(const Matrix &M) : _lines(M.lines()), _columns(M.columns()) {
...
Matrix Matrix::identity(unsigned size) { return diag(size, 1); }
...
Matrix Matrix::operator-(float k) const { return (*this) + (-k); }
