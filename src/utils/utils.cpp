...
Position Utils::projectPointAtLine(const Position &s1, const Position &s2, const Position &point) {
    ...
    return {true, s1.x() + scalar * bUnitary.x(), s1.y() + scalar * bUnitary.y()};
}

Position Utils::projectPointAtSegment(const Position &s1, const Position &s2,
...
bool Utils::isInsideBigArea(VSSRef::Color teamColor, Position pos) {
    if (!getConstants()->is5v5())
        return isInsideGoalArea(teamColor, pos);
...
void Utils::setConstants(Constants *constants) { _constants = constants; }

void Utils::setField(Field *field) { _field = field; }

Constants *Utils::getConstants() {
    if (_constants == nullptr) {
        std::cout << Text::red("[ERROR] ", true)
                  << Text::bold("Constants with nullptr value at Utils") + '\n';
    } else {
        return _constants;
    }
...
Field *Utils::getField() {
    if (_field == nullptr) {
        std::cout << Text::red("[ERROR] ", true)
                  << Text::bold("Field with nullptr value at Utils") + '\n';
    } else {
        return _field;
    }
